...
locals {
  ...
  secretsmanager_arns = compact([for arn in local.all_secret_arns : strcontains(arn, ":secretsmanager:") ? arn : null])
  ssm_parameter_arns  = compact([for arn in local.all_secret_arns : strcontains(arn, ":ssm:") ? arn : null])
}
